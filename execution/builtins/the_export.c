...
/*   Updated: 2024/10/16 15:47:03 by rallali          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char *equal_pos = strchr(cmd, '=');
	char *key = extract_key(cmd, equal_pos);
	char *value = extract_value(equal_pos);
...
void the_export(t_command *cmd, t_env **env)
{
	...
	while (cmd->args[i]) 
	{
		int append = check_append(cmd->args[i]);
